...
public class Converter implements TextGraphicsConverter {
    ...
    public String convert(String url) throws IOException, BadImageSizeException {
        ...
        if (((width < img.getWidth()) || (height < img.getHeight())) & (width * height > 0)) {
            double sizeCorrectByWidth = (double) img.getWidth() / width;
            double sizeCorrectByHeight = (double) img.getHeight() / height;
            if (sizeCorrectByWidth > sizeCorrectByHeight) {
                width = (int) (img.getWidth() / sizeCorrectByWidth);
                height = (int) (img.getHeight() / sizeCorrectByWidth);
            } else {
                width = (int) (width /sizeCorrectByHeight);
                height = (int) (height / sizeCorrectByHeight);
            }
            newWidth = width;
            newHeight = height;
        } else {
        ...
        char[][] pixelArray = new char[newWidth][newHeight];
        ...
        for (int h = 0; h < bwImg.getHeight() ; h++) {
            for (int w = 0; w < bwImg.getWidth(); w++) {
                //int color = bwRaster.getPixel(w, h, tmpArray)[0];
                int color = bwRaster.getPixel(w, h, new int[3])[0];
                char c = schema.convert(color);
                pixelArray[w][h] = c; //запоминаем символ c, например, в двумерном массиве или как-то ещё на ваше усмотрение
            }
        }
